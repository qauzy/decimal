...
// BigDecimal represents a fixed-point decimal. It is immutable.
// number = value * 10 ^ exp
...
// NewFromString returns a new BigDecimal from a string representation.
...
func NewFromString(value string) (BigDecimal, error) {
	...
	if eIndex != -1 {
		...
		if err != nil {
			if e, ok := err.(*strconv.NumError); ok && e.Err == strconv.ErrRange {
				return BigDecimal{}, fmt.Errorf("can't convert %s to decimal: fractional part too long", value)
			}
			return BigDecimal{}, fmt.Errorf("can't convert %s to decimal: exponent is not numeric", value)
		}
		value = value[:eIndex]
		exp = expInt
	}
	...
	for i := 0; i < vLen; i++ {
		if value[i] == '.' {
			if pIndex > -1 {
				return BigDecimal{}, fmt.Errorf("can't convert %s to decimal: too many .s", value)
			}
			pIndex = i
		}
	}
	...
	if len(intString) <= 18 {
		...
		if err != nil {
			return BigDecimal{}, fmt.Errorf("can't convert %s to decimal", value)
		}
	...
	if exp < math.MinInt32 || exp > math.MaxInt32 {
		...
		return BigDecimal{}, fmt.Errorf("can't convert %s to decimal: fractional part too long", originalInput)
	}
...
// NewFromFormattedString returns a new BigDecimal from a formatted string representation.
...
// RequireFromString returns a new BigDecimal from a string representation
...
func newFromFloat(val float64, bits uint64, flt *floatInfo) BigDecimal {
	if math.IsNaN(val) || math.IsInf(val, 0) {
		panic(fmt.Sprintf("Cannot create a BigDecimal from %v", val))
	}
	...
	if d.nd < 19 {
		...
		return BigDecimal{value: big.NewInt(tmp), exp: int32(d.dp) - int32(d.nd)}
	}
	dValue := new(big.Int)
	dValue, ok := dValue.SetString(string(d.d[:d.nd]), 10)
	if ok {
		return BigDecimal{value: dValue, exp: int32(d.dp) - int32(d.nd)}
	}

	return NewFromFloatWithExponent(val, int32(d.dp)-int32(d.nd))
}

// NewFromFloatWithExponent converts a float64 to BigDecimal, with an arbitrary
...
func NewFromFloatWithExponent(value float64, exp int32) BigDecimal {
	if math.IsNaN(value) || math.IsInf(value, 0) {
		panic(fmt.Sprintf("Cannot create a BigDecimal from %v", value))
	}
...
// than the initial exponent of the BigDecimal.
...
//	1.2000
//
func (d BigDecimal) rescale(exp int32) BigDecimal {
	d.ensureInitialized()
...
func (d BigDecimal) QuoRem(d2 BigDecimal, precision int32) (BigDecimal, BigDecimal) {
	...
	var q, r big.Int
	q.QuoRem(&aa, &bb, &r)
	dq := BigDecimal{value: &q, exp: scale}
	dr := BigDecimal{value: &r, exp: scalerest}
	return dq, dr
}
...
func (d BigDecimal) DivRound(d2 BigDecimal, precision int32) BigDecimal {
	...
	r2 := BigDecimal{value: &rv2, exp: r.exp + precision}
...
func (d BigDecimal) ExpHullAbrham(overallPrecision uint32) (BigDecimal, error) {
	...
	if d.IsZero() {
		return BigDecimal{oneInt, 0}, nil
	}
	...
	if d.Abs().Cmp(overflowThreshold) > 0 {
		return BigDecimal{}, fmt.Errorf("over/underflow threshold, exp(x) cannot be calculated precisely")
	}
	...
	if d.Abs().Cmp(overflowThreshold2) <= 0 {
		return BigDecimal{oneInt, d.exp}, nil
	}
	...
	r := BigDecimal{new(big.Int).Set(d.value), d.exp - t} // reduced argument
	...
	if nf > float64(ExpMaxIterations) || math.IsNaN(nf) {
		return BigDecimal{}, fmt.Errorf("exact value cannot be calculated in <=ExpMaxIterations iterations")
	}
	n := int64(nf)
...
func (d BigDecimal) ExpTaylor(precision int32) (BigDecimal, error) {
	...
	if d.IsZero() {
		return BigDecimal{oneInt, 0}.Round(precision), nil
	}
...
func (d BigDecimal) Coefficient() *big.Int {
	d.ensureInitialized()
	// we copy the coefficient so that mutating the result does not mutate the BigDecimal.
...
func (d BigDecimal) RoundCash(interval uint8) BigDecimal {
	...
	case 100:
		iVal = oneInt
	default:
		panic(fmt.Sprintf("BigDecimal does not support this Cash rounding interval `%d`. Supported: 5, 10, 25, 50, 100", interval))
	}
	dVal := BigDecimal{
		value: iVal,
	}
...
func (d BigDecimal) Floor() BigDecimal {
	...
	return BigDecimal{value: z, exp: 0}
}
...
func (d BigDecimal) string(trimTrailingZeros bool) string {
	if d.exp >= 0 {
...
func (d *NullDecimal) Scan(value interface{}) error {
	...
	d.Valid = true
	return d.BigDecimal.Scan(value)
}
...
func (d *NullDecimal) UnmarshalJSON(decimalBytes []byte) error {
	...
	d.Valid = true
	return d.BigDecimal.UnmarshalJSON(decimalBytes)
}
...
func (d NullDecimal) MarshalJSON() ([]byte, error) {
	...
	return d.BigDecimal.MarshalJSON()
}
...
func (d *NullDecimal) UnmarshalText(text []byte) error {
	...
	if err := d.BigDecimal.UnmarshalText(text); err != nil {
		d.Valid = false
		return err
	}
	d.Valid = true
...
func (d NullDecimal) MarshalText() (text []byte, err error) {
	...
	return d.BigDecimal.MarshalText()
}
...
func (d BigDecimal) xatan() BigDecimal {
...
func (d BigDecimal) satan() BigDecimal {
...
var _tanP = [...]BigDecimal{
	NewFromFloat(-1.30936939181383777646e+4), // 0xc0c992d8d24f3f38
	NewFromFloat(1.15351664838587416140e+6),  // 0x413199eca5fc9ddd
	NewFromFloat(-1.79565251976484877988e+7), // 0xc1711fead3299176
}
var _tanQ = [...]BigDecimal{
